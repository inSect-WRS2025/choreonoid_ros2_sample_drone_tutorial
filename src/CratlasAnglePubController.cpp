...
#include <geometry_msgs/msg/twist.hpp> // Twistメッセージ型は引き続き使用
#include <memory>
#include <thread>
#include <mutex>

class PanTiltPublisherController : public cnoid::SimpleController
{
public:
...
CNOID_IMPLEMENT_SIMPLE_CONTROLLER_FACTORY(PanTiltPublisherController)

bool PanTiltPublisherController::configure(cnoid::SimpleControllerConfig* config)
{
...
bool PanTiltPublisherController::initialize(cnoid::SimpleControllerIO* io)
{
    currentMode = DroneMode;  // 初期状態はドローン制御のまま
    return true;
}

bool PanTiltPublisherController::control()
{
...
void PanTiltPublisherController::unconfigure()
{
    if(executor) {
        executor->cancel();
        executorThread.join();
